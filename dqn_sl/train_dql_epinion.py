...
u = tf.cond(start_s > tf.constant(0.0), lambda: action_opinion[2], lambda: cur_opinion[1] + cur_opinion[2] + cur_opinion[0] * action_opinion[2])
...
reward = tf.cond(target_s > tf.constant(0.0), lambda: reward_old+1.0, lambda: reward_2)
...
# l2 = weight_decay * (tf.nn.l2_loss(variables[0]) + tf.nn.l2_loss(variables[1]) + tf.nn.l2_loss(variables[2]))
l2 = weight_decay * (tf.nn.l2_loss(W1) + tf.nn.l2_loss(W2) + tf.nn.l2_loss(W3))
loss = score + l2
# loss = score
...
for o in range(1):
    ...
    with tf.Session(config=config) as sess:
        ...
        for episode in range(episodes):
            ...
            epsilon = max((1.0 - float(episode)/float(200)), 0.1)
            for epoch in range(training_epochs):
                ...
                output_, input_1_, l2_n = sess.run([output, input_1, l2], feed_dict={feature: f, opinion_all: opinion, adj_opinion: adj, state: state_n, target_node: target_, start_node: start_, neigh_state: neigh_s})
                action_r = greed2(epsilon, neigh, state_n, output_)
                ...
                if neigh[action_r] == []:
                    ...
                    neigh_status = -1.0
                outs = sess.run([train_step, score, action_next, b, d, u, loss, action_opinion, max_q2, input_2, output2, reward, y_i],
                                feed_dict={feature: f, opinion_all: opinion, cur_opinion: current_opinion, adj_opinion: adj, state: state_n, target_node: target_, start_node: start_, W11: weight[0], neigh_action: neigh_a,
                ...
                state_n = action_r
                path.append(state_n)
                print("episode = ", episode, "loss: ", outs[6][0], "action = ", state_n)
                if state_n == target_:
                    print("find path:", path)
                    if path not in path_ex['paths']:
                        path_ex['paths'].append(path)
                        path_ex['opinion'].append(current_opinion)
                    break
                if epoch == 20:
                    pass
        ...
        for i in range(len(E_X)):
            ...
            while state_n != target_:
                ...
                path1.append(state_n)
            print path1
            pred_op = get_path_opinion(path1, opinion)
            print("ground truth:", opinion[start_n][target_], "predict opinion:", pred_op)
